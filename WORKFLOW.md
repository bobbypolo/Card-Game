...
3. **For each story** (no user interaction):
   ...
   - Dispatches a `ralph-story` sub-agent (which manages worker, QA, and merge)
...
STEP 4:   Dispatch ralph-story agent (handles worker, QA, merge internally)
           ralph-story returns RALPH_STORY_RESULT
