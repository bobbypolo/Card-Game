...
1. **Sparse checkout (if scope provided)**: If the dispatch prompt includes a `scope` field with directory paths, apply git sparse-checkout to limit the working tree to relevant directories:

   ```bash
   git sparse-checkout init --cone
   git sparse-checkout set .claude/ CLAUDE.md pyproject.toml [scope_dirs...]
   ```

   Where `[scope_dirs...]` are the directory paths from the `scope` field. This ensures the worker only checks out files relevant to this story.
   If sparse-checkout fails or `scope` is absent/empty: log a warning and continue with the full checkout (fallback).
