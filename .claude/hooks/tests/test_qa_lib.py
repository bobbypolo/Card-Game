"""Tests for _qa_lib.py -- QA engine utilities."""
...
from _qa_lib import (
    ...
    estimate_story_complexity,  # noqa: F401 -- used in TestEstimateStoryComplexity* classes
    evaluate_promotion,
    extract_plan_r_markers,
    parse_plan_changes,
    scan_test_quality,
    validate_prd_story_schema,
    validate_r_markers,
)
...
class TestValidateRMarkersACLevel:
    """Tests for AC-level IDs (R-P0-01-AC1) in addition to story-level (R-P0-01)."""

    def test_ac_level_ids_matched(self, tmp_path: Path) -> None:
        ac1 = "R-P1" + "-01-AC1"  # Split to avoid scanner matching this file
        ac2 = "R-P1" + "-01-AC2"
        test_file = tmp_path / "test_ac.py"
        test_file.write_text(
            f'def test_ac1():\n    """# Tests {ac1}"""\n    assert True\n'
            f'def test_ac2():\n    """# Tests {ac2}"""\n    assert True\n',
            encoding="utf-8",
        ...
        result = validate_r_markers(tmp_path, prd)
        assert "R-P1-01-AC1" in result["markers_found"]
        assert "R-P1-01-AC2" in result["markers_found"]
        assert result["result"] == "PASS"

    def test_ac_level_missing_detected(self, tmp_path: Path) -> None:
        ac1 = "R-P1" + "-01-AC1"
        ...
        result = validate_r_markers(tmp_path, prd)
        assert "R-P1-01-AC1" in result["markers_valid"]
        assert "R-P1-01-AC2" in result["missing_markers"]
        assert result["result"] == "FAIL"

    def test_mixed_story_and_ac_level(self, tmp_path: Path) -> None:
        story_id = "R-P1" + "-01"
        ac_id = "R-P1" + "-02-AC1"
        ...
        result = validate_r_markers(tmp_path, prd)
        assert "R-P1-01" in result["markers_valid"]
        assert "R-P1-02-AC1" in result["markers_valid"]
        assert result["result"] == "PASS"
...
class TestPlanSync:
    ...
    def test_check_plan_prd_sync_missing_prd(self, tmp_path: Path) -> None:
        # Missing prd.json is a fail-closed parse error (in_sync=False).
        plan = tmp_path / "PLAN.md"
        plan.write_text("- R-P1-01: criterion\n", encoding="utf-8")
    ...
    def test_check_plan_prd_sync_invalid_json_returns_parse_error(
        self, tmp_path: Path
    ) -> None:
        """# Tests R-P6-03 -- invalid JSON in prd.json returns in_sync=False error_kind=parse_error."""
        plan = tmp_path / "PLAN.md"
        plan.write_text("- R-P1-01: criterion\n", encoding="utf-8")
        prd = tmp_path / "prd.json"
        prd.write_text("{not valid json", encoding="utf-8")
        result = check_plan_prd_sync(plan, prd)
        assert result["in_sync"] is False
        assert result["error_kind"] == "parse_error"

    def test_check_plan_prd_sync_missing_schema_keys_returns_schema_error(
        self, tmp_path: Path
    ) -> None:
        """# Tests R-P6-04 -- prd.json with invalid schema (non-dict root) returns in_sync=False error_kind=schema_error."""
        plan = tmp_path / "PLAN.md"
        plan.write_text("- R-P1-01: criterion\n", encoding="utf-8")
        prd = tmp_path / "prd.json"
        # Valid JSON but root is an array, not an object — invalid schema
        prd.write_text('[{"id": "R-P1-01"}]', encoding="utf-8")
        result = check_plan_prd_sync(plan, prd)
        assert result["in_sync"] is False
...
class TestComputePlanHash:
    """Tests for compute_plan_hash() -- normalized criteria-line hashing."""
    ...
    def test_hash_order_independent(self, tmp_path: Path) -> None:
        """R-marker lines in different order produce same hash (sorted internally)."""
        plan_a = tmp_path / "a.md"
        plan_b = tmp_path / "b.md"
    ...
    def test_sync_uses_normalized_hash(self, tmp_path: Path) -> None:
        """check_plan_prd_sync returns compute_plan_hash output."""
        plan = tmp_path / "PLAN.md"
...
class TestPrdSchema:
    """Tests for prd.json story schema validation (v2.1 optional fields)."""

    def test_story_with_depends_on_and_parallel_group_is_valid(self) -> None:
        """R-P7-01: story with dependsOn=[] and parallelGroup=null is valid."""
        story = {
            "id": "STORY-001",
            "dependsOn": [],
            "parallelGroup": None,
        }
        errors = validate_prd_story_schema(story)
        error_count = len(errors)
        assert error_count == 0

    def test_story_without_optional_fields_is_valid(self) -> None:
        """R-P7-02: story without dependsOn or parallelGroup is backward-compatible."""
        story = {
            "id": "STORY-001",
            "description": "A legacy story without new fields",
            "phase": 1,
            "passed": False,
        }
        errors = validate_prd_story_schema(story)
    ...
    def test_depends_on_as_array_of_strings_is_valid(self) -> None:
        """R-P7-01: dependsOn as array of story ID strings passes validation."""
        story = {
            "id": "STORY-002",
            "dependsOn": ["STORY-001"],
            "parallelGroup": None,
    ...
    def test_parallel_group_as_integer_is_valid(self) -> None:
        """R-P7-01: parallelGroup as integer passes validation."""
        story = {
            "id": "STORY-003",
            "dependsOn": [],
            "parallelGroup": 1,
    ...
    def test_depends_on_defaults_via_get(self) -> None:
        """R-P7-02: .get() returns [] default when dependsOn absent."""
        story: dict = {"id": "STORY-001"}
        default_val = story.get("dependsOn", [])
        assert default_val == []

    def test_parallel_group_defaults_via_get(self) -> None:
        """R-P7-02: .get() returns None default when parallelGroup absent."""
        story: dict = {"id": "STORY-001"}
        default_val = story.get("parallelGroup", None)
        assert default_val is None


class TestPrdSchemaV22Scope:
    """Tests for prd.json v2.2 scope field validation.

    # Tests R-P3-01
    """

    def test_scope_as_list_of_strings_is_valid(self) -> None:
        """# Tests R-P3-01 -- scope as list of strings returns empty error list."""
        story = {"id": "STORY-001", "scope": [".claude/hooks/", "src/"]}
        errors = validate_prd_story_schema(story)
        assert errors == []

    def test_scope_as_empty_list_is_valid(self) -> None:
        """# Tests R-P3-01 -- scope as empty list returns empty error list."""
        story = {"id": "STORY-001", "scope": []}
        errors = validate_prd_story_schema(story)
        assert errors == []

    def test_scope_as_non_list_returns_error(self) -> None:
        """# Tests R-P3-01 -- scope as non-list returns error."""
        story = {"id": "STORY-001", "scope": "src/"}
        errors = validate_prd_story_schema(story)
        assert len(errors) == 1
        assert "scope" in errors[0]
        assert "list" in errors[0]

    def test_scope_with_non_string_element_returns_error(self) -> None:
        """# Tests R-P3-01 -- scope list containing non-string returns error."""
        story = {"id": "STORY-001", "scope": ["src/", 42]}
        errors = validate_prd_story_schema(story)
        assert len(errors) == 1
        assert "scope[1]" in errors[0]
        assert "string" in errors[0]

    def test_scope_absent_is_valid(self) -> None:
        """# Tests R-P3-01 -- scope absent returns empty error list (backward compat)."""
        story = {"id": "STORY-001"}
        errors = validate_prd_story_schema(story)
        assert errors == []


class TestPrdSchemaV22Component:
    """Tests for prd.json v2.2 component field validation.

    # Tests R-P3-02
    """

    def test_component_as_string_is_valid(self) -> None:
        """# Tests R-P3-02 -- component as string returns empty error list."""
        story = {"id": "STORY-001", "component": "qa-engine"}
        errors = validate_prd_story_schema(story)
        assert errors == []

    def test_component_as_non_string_returns_error(self) -> None:
        """# Tests R-P3-02 -- component as non-string returns error."""
        story = {"id": "STORY-001", "component": 123}
        errors = validate_prd_story_schema(story)
        assert len(errors) == 1
        assert "component" in errors[0]
        assert "string" in errors[0]

    def test_component_absent_is_valid(self) -> None:
        """# Tests R-P3-02 -- component absent returns empty error list (backward compat)."""
        story = {"id": "STORY-001"}
        errors = validate_prd_story_schema(story)
        assert errors == []


class TestPrdSchemaV22Complexity:
    """Tests for prd.json v2.2 complexity field validation.

    # Tests R-P3-03
    """

    def test_complexity_simple_is_valid(self) -> None:
        """# Tests R-P3-03 -- complexity='simple' returns empty error list."""
        story = {"id": "STORY-001", "complexity": "simple"}
        errors = validate_prd_story_schema(story)
        assert errors == []

    def test_complexity_medium_is_valid(self) -> None:
        """# Tests R-P3-03 -- complexity='medium' returns empty error list."""
        story = {"id": "STORY-001", "complexity": "medium"}
        errors = validate_prd_story_schema(story)
        assert errors == []

    def test_complexity_complex_is_valid(self) -> None:
        """# Tests R-P3-03 -- complexity='complex' returns empty error list."""
        story = {"id": "STORY-001", "complexity": "complex"}
        errors = validate_prd_story_schema(story)
        assert errors == []

    def test_complexity_invalid_value_returns_error(self) -> None:
        """# Tests R-P3-03 -- complexity with invalid value returns error."""
        story = {"id": "STORY-001", "complexity": "extreme"}
        errors = validate_prd_story_schema(story)
        assert len(errors) == 1
        assert "complexity" in errors[0]

    def test_complexity_absent_is_valid(self) -> None:
        """# Tests R-P3-03 -- complexity absent returns empty error list (backward compat)."""
        story = {"id": "STORY-001"}
        errors = validate_prd_story_schema(story)
        assert errors == []


class TestPrdSchemaV22MaxTurns:
    """Tests for prd.json v2.2 maxTurns field validation."""

    def test_max_turns_positive_integer_is_valid(self) -> None:
        """-- maxTurns as positive integer returns empty error list."""
        story = {"id": "STORY-001", "maxTurns": 150}
        errors = validate_prd_story_schema(story)
        assert errors == []

    def test_max_turns_as_float_returns_error(self) -> None:
        """-- maxTurns as float (non-integer) returns error."""
        story = {"id": "STORY-001", "maxTurns": 150.5}
        errors = validate_prd_story_schema(story)
        assert len(errors) == 1
        assert "maxTurns" in errors[0]

    def test_max_turns_as_string_returns_error(self) -> None:
        """-- maxTurns as string returns error."""
        story = {"id": "STORY-001", "maxTurns": "150"}
        errors = validate_prd_story_schema(story)
        assert len(errors) == 1
        assert "maxTurns" in errors[0]

    def test_max_turns_zero_returns_error(self) -> None:
        """-- maxTurns=0 (non-positive) returns error."""
        story = {"id": "STORY-001", "maxTurns": 0}
        errors = validate_prd_story_schema(story)
        assert len(errors) == 1
        assert "maxTurns" in errors[0]

    def test_max_turns_negative_returns_error(self) -> None:
        """-- maxTurns<0 (non-positive) returns error."""
        story = {"id": "STORY-001", "maxTurns": -5}
        errors = validate_prd_story_schema(story)
        assert len(errors) == 1
        assert "maxTurns" in errors[0]

    def test_max_turns_absent_is_valid(self) -> None:
        """-- maxTurns absent returns empty error list (backward compat)."""
        story = {"id": "STORY-001"}
        errors = validate_prd_story_schema(story)
        assert errors == []


class TestPrdSchemaV22BackwardCompat:
    """Tests for prd.json v2.2 backward compatibility with v2.0 stories."""

    def test_v20_story_without_new_fields_is_valid(self) -> None:
        """-- v2.0 story without scope/component/complexity/maxTurns is valid."""
        story = {
            "id": "STORY-001",
            "description": "A legacy v2.0 story",
            "phase": 1,
            "acceptanceCriteria": [
                {"id": "R-P1-01", "criterion": "...", "testType": "unit"}
            ],
            "gateCmds": {"unit": "pytest tests/"},
            "passed": False,
    ...
    def test_minimal_story_without_any_optional_fields_is_valid(self) -> None:
        """-- minimal story dict returns empty error list."""
        story: dict = {}
        errors = validate_prd_story_schema(story)
        assert errors == []


class TestEstimateStoryComplexitySimple:
    """Tests for estimate_story_complexity() returning simple classification."""

    def test_simple_classification_small_scope_few_criteria(self) -> None:
        """-- 3 criteria + 1 scope dir = score 4 → simple."""
        story = {
            "acceptanceCriteria": [
                {"id": "R-P1-01"},
                {"id": "R-P1-02"},
        ...
        result = estimate_story_complexity(story)
        assert result["complexity"] == "simple"
        assert result["score"] <= 6

    def test_simple_classification_returns_max_turns(self) -> None:
        """-- simple classification returns positive maxTurns."""
        story = {
            "acceptanceCriteria": [{"id": "R-P1-01"}, {"id": "R-P1-02"}],
            "scope": ["src/"],
        }
        result = estimate_story_complexity(story)
        assert result["complexity"] == "simple"
        assert result["maxTurns"] > 0

    def test_simple_returns_score_key(self) -> None:
        """-- result always includes score key with non-negative int value."""
        story = {"acceptanceCriteria": [{"id": "R-P1-01"}], "scope": ["x/"]}
        result = estimate_story_complexity(story)
        assert "score" in result
        assert result["score"] >= 0


class TestEstimateStoryComplexityMedium:
    """Tests for estimate_story_complexity() returning medium classification."""

    def test_medium_classification_five_criteria_four_scope_dirs(self) -> None:
        """-- 5 criteria + 4 scope dirs = score 9 → medium."""
        story = {
            "acceptanceCriteria": [
                {"id": "R-P1-01"},
                {"id": "R-P1-02"},
        ...
        assert 7 <= result["score"] <= 12

    def test_medium_returns_expected_score(self) -> None:
        """-- score for medium case equals criteria+scope count (no cross-pkg)."""
        # Use sub-dirs under the same top-level package to avoid auto cross-package detection
        story = {
            "acceptanceCriteria": [{"id": f"R-P1-{i:02d}"} for i in range(5)],
            "scope": ["pkg/sub_a/", "pkg/sub_b/"],
        }
        result = estimate_story_complexity(story)
        # 5 criteria + 2 scope dirs = 7, no cross-package → score 7 (medium)
        assert result["score"] == 7
        assert result["complexity"] == "medium"


class TestEstimateStoryComplexityComplex:
    """Tests for estimate_story_complexity() returning complex classification."""

    def test_complex_classification_eight_criteria_five_dirs_cross_package(
        self,
    ) -> None:
        """-- 8 criteria + 5 dirs + cross_package=True = score 16 → complex."""
        story = {
            "acceptanceCriteria": [{"id": f"R-P1-{i:02d}"} for i in range(8)],
            "scope": ["a/", "b/", "c/", "d/", "e/"],
        }
        result = estimate_story_complexity(story, cross_package=True)
        assert result["complexity"] == "complex"
        assert result["score"] > 12

    def test_complex_score_includes_cross_package_bonus(self) -> None:
        """-- cross_package=True adds +3 to score when not auto-detected."""
        # Use sub-dirs under the same top-level package so auto-detection doesn't fire
        story = {
            "acceptanceCriteria": [{"id": f"R-P1-{i:02d}"} for i in range(8)],
            "scope": ["pkg/sub_a/", "pkg/sub_b/"],
        }
        result_no_cp = estimate_story_complexity(story, cross_package=False)
        result_with_cp = estimate_story_complexity(story, cross_package=True)
        # Without cross-package: 8 + 2 = 10; with: 10 + 3 = 13
        assert result_with_cp["score"] == result_no_cp["score"] + 3

    def test_complex_auto_detected_when_more_than_two_top_dirs(self) -> None:
        """-- cross-package auto-detected when scope has >2 unique top dirs."""
        story = {
            "acceptanceCriteria": [{"id": f"R-P1-{i:02d}"} for i in range(8)],
            "scope": ["pkg_a/sub/", "pkg_b/sub/", "pkg_c/sub/"],
        }
        result = estimate_story_complexity(story)
        assert result["score"] == 8 + 3 + 3


class TestHeavyMockDetection:
    """Tests that scan_test_quality detects heavily-mocked tests."""

    def test_detects_heavy_mock_test(self, tmp_path: Path) -> None:
        """Test with >80% mocked dependencies is flagged."""
        code = tmp_path / "test_heavy.py"
        code.write_text(
            "from unittest.mock import patch, MagicMock\n\n"
            "def test_heavily_mocked():\n"
            '    with patch("mod.dep_a") as a:\n'
            '        with patch("mod.dep_b") as b:\n'
            '            with patch("mod.dep_c") as c:\n'
            '                with patch("mod.dep_d") as d:\n'
            "                    a.return_value = 1\n"
            "                    b.return_value = 2\n"
            "                    assert a.called or b.called\n",
            encoding="utf-8",
        )
        result = scan_test_quality(code)
        assert len(result["heavy_mock_tests"]) >= 1
        assert result["quality_score"] == "FAIL"

    def test_normal_mock_usage_not_flagged(self, tmp_path: Path) -> None:
        """Test with <4 mocks is not flagged as heavy-mock."""
        code = tmp_path / "test_normal_mock.py"
        code.write_text(
            "from unittest.mock import patch\n\n"
            "def test_with_one_mock():\n"
            '    with patch("mod.external_api") as api:\n'
            "        api.return_value = 42\n"
            "        result = process(api())\n"
            "        assert result == 42\n",
            encoding="utf-8",
        ...
        result = scan_test_quality(code)
        assert result["heavy_mock_tests"] == []

    def test_heavy_mock_field_always_present(self, tmp_path: Path) -> None:
        """Output always includes heavy_mock_tests field."""
        code = tmp_path / "test_simple.py"
        code.write_text(
            "def test_add():\n    assert 1 + 1 == 2\n",
            encoding="utf-8",
        )
        result = scan_test_quality(code)
        assert "heavy_mock_tests" in result


class TestCheckNegativeTestsEnforcement:
    """Tests that check_negative_tests returns WARN for validation criteria without negative tests."""

    def test_validation_criterion_without_negative_tests_warns(self) -> None:
        """Criterion with 'validate' keyword but no negative tests returns WARN."""
        result = check_negative_tests(
            "validate input format and reject malformed data",
            ["test_create_item", "test_get_item"],
        )
        assert result["needs_negative"] is True
        assert result["has_negative"] is False
        assert result["result"] == "WARN"
    ...
    def test_validation_criterion_with_negative_tests_passes(self) -> None:
        """Criterion with 'validate' keyword and negative tests returns PASS."""
        result = check_negative_tests(
            "validate input format",
            ["test_create_item", "test_invalid_input"],
        )
        assert result["needs_negative"] is True
        assert result["has_negative"] is True
        assert result["result"] == "PASS"
    ...
    def test_non_validation_criterion_always_passes(self) -> None:
        """Criterion without validation keywords always PASS."""
        result = check_negative_tests(
...
# Tests R-P10-05
class TestValidateStoryPromotion:
    """Tests for validate_story_promotion() in _qa_lib.py."""

    def test_pass_when_receipt_pass_and_reviewer_pass(self, tmp_path: Path) -> None:
        """Returns (True, '') when receipt has PASS and reviewer is PASS."""
        from _qa_lib import validate_story_promotion

        receipt_path = tmp_path / "qa-receipt.json"
        receipt_path.write_text(
            json.dumps({"overall_result": "PASS", "criteria_verified": []}),
            encoding="utf-8",
        )
        ok, reason = validate_story_promotion(receipt_path, "PASS")
        assert ok is True
        assert reason == ""

    def test_pass_when_receipt_pass_and_reviewer_warn(self, tmp_path: Path) -> None:
        """Returns (True, '') when receipt has PASS and reviewer is WARN (not FAIL)."""
        from _qa_lib import validate_story_promotion
        ...
        ok, reason = validate_story_promotion(receipt_path, "WARN")
        assert ok is True
        assert reason == ""

    def test_fail_when_reviewer_is_fail(self, tmp_path: Path) -> None:
        """Returns (False, reason) when reviewer_result is FAIL, even if receipt passes."""
        from _qa_lib import validate_story_promotion
        ...
        ok, reason = validate_story_promotion(receipt_path, "FAIL")
        assert ok is False
        assert reason != ""

    def test_fail_when_receipt_overall_not_pass(self, tmp_path: Path) -> None:
        """Returns (False, reason) when receipt overall_result != PASS."""
        from _qa_lib import validate_story_promotion

        receipt_path = tmp_path / "qa-receipt.json"
        receipt_path.write_text(
            json.dumps({"overall_result": "FAIL"}),
            encoding="utf-8",
        )
        ok, reason = validate_story_promotion(receipt_path, "PASS")
        assert ok is False
    ...
    def test_fail_when_receipt_missing(self, tmp_path: Path) -> None:
        """Returns (False, reason) when receipt file does not exist."""
        from _qa_lib import validate_story_promotion

        receipt_path = tmp_path / "nonexistent-receipt.json"
        ok, reason = validate_story_promotion(receipt_path, "PASS")
        assert ok is False
        assert reason != ""

    def test_fail_when_receipt_invalid_json(self, tmp_path: Path) -> None:
        """Returns (False, reason) when receipt file contains invalid JSON."""
        from _qa_lib import validate_story_promotion
    ...
    def test_fail_when_receipt_missing_overall_result_key(self, tmp_path: Path) -> None:
        """Returns (False, reason) when receipt JSON has no overall_result key."""
        from _qa_lib import validate_story_promotion

        receipt_path = tmp_path / "qa-receipt.json"
        receipt_path.write_text(
            json.dumps({"steps": [], "criteria_verified": []}),
            encoding="utf-8",
        )
        ok, reason = validate_story_promotion(receipt_path, "PASS")
        assert ok is False
...
class TestEvaluatePromotion:
    """Tests for evaluate_promotion() -- typed PromotionGate wrapper (R-P1-01 through R-P1-05)."""

    def test_allowed_when_receipt_pass_and_reviewer_pass(self, tmp_path: Path) -> None:
        """Returns PromotionDecision(allowed=True, status=ALLOWED, failure_reason=None)."""
        receipt_path = tmp_path / "qa-receipt.json"
        receipt_path.write_text(
            json.dumps({"overall_result": "PASS", "criteria_verified": []}),
            encoding="utf-8",
        )
        inputs = PromotionInputs(receipt_path=receipt_path, reviewer_result="PASS")
        decision = evaluate_promotion(inputs)
        assert decision.allowed is True
        assert decision.status == PromotionStatus.ALLOWED
        assert decision.failure_reason is None

    def test_allowed_when_receipt_pass_and_reviewer_warn(self, tmp_path: Path) -> None:
        """WARN reviewer is not FAIL so promotion is allowed."""
        receipt_path = tmp_path / "qa-receipt.json"
        receipt_path.write_text(
            json.dumps({"overall_result": "PASS"}),
            encoding="utf-8",
        )
        inputs = PromotionInputs(receipt_path=receipt_path, reviewer_result="WARN")
        decision = evaluate_promotion(inputs)
        assert decision.allowed is True
        assert decision.status == PromotionStatus.ALLOWED
        assert decision.failure_reason is None

    def test_blocked_when_receipt_not_found(self, tmp_path: Path) -> None:
        """RECEIPT_NOT_FOUND when receipt path does not exist."""
        receipt_path = tmp_path / "nonexistent-receipt.json"
        inputs = PromotionInputs(receipt_path=receipt_path, reviewer_result="PASS")
        decision = evaluate_promotion(inputs)
        assert decision.allowed is False
        assert decision.failure_reason == PromotionFailureReason.RECEIPT_NOT_FOUND
        assert decision.status == PromotionStatus.BLOCKED

    def test_blocked_when_receipt_invalid_json(self, tmp_path: Path) -> None:
        """RECEIPT_PARSE_ERROR when receipt contains invalid JSON."""
        receipt_path = tmp_path / "qa-receipt.json"
        receipt_path.write_text("not valid json {{", encoding="utf-8")
        inputs = PromotionInputs(receipt_path=receipt_path, reviewer_result="PASS")
        decision = evaluate_promotion(inputs)
        assert decision.allowed is False
        assert decision.failure_reason == PromotionFailureReason.RECEIPT_PARSE_ERROR
        assert decision.status == PromotionStatus.BLOCKED

    def test_blocked_when_receipt_not_passed(self, tmp_path: Path) -> None:
        """RECEIPT_NOT_PASSED when overall_result != PASS."""
        receipt_path = tmp_path / "qa-receipt.json"
        receipt_path.write_text(
            json.dumps({"overall_result": "FAIL"}),
            encoding="utf-8",
        ...
        decision = evaluate_promotion(inputs)
        assert decision.allowed is False
        assert decision.failure_reason == PromotionFailureReason.RECEIPT_NOT_PASSED
        assert decision.status == PromotionStatus.BLOCKED

    def test_blocked_when_receipt_missing_overall_result(self, tmp_path: Path) -> None:
        """RECEIPT_NOT_PASSED when overall_result key is absent (None != PASS)."""
        receipt_path = tmp_path / "qa-receipt.json"
        receipt_path.write_text(
            json.dumps({"steps": [], "criteria_verified": []}),
            encoding="utf-8",
    ...
    def test_blocked_when_reviewer_rejected(self, tmp_path: Path) -> None:
        """REVIEWER_REJECTED when reviewer_result == FAIL."""
        receipt_path = tmp_path / "qa-receipt.json"
        receipt_path.write_text(
            json.dumps({"overall_result": "PASS"}),
            encoding="utf-8",
        )
        inputs = PromotionInputs(receipt_path=receipt_path, reviewer_result="FAIL")
        decision = evaluate_promotion(inputs)
        assert decision.allowed is False
        assert decision.failure_reason == PromotionFailureReason.REVIEWER_REJECTED
        assert decision.status == PromotionStatus.BLOCKED

    def test_returns_promotion_decision_type(self, tmp_path: Path) -> None:
        """evaluate_promotion returns a PromotionDecision instance."""
        receipt_path = tmp_path / "qa-receipt.json"
        receipt_path.write_text(
            json.dumps({"overall_result": "PASS"}),
            encoding="utf-8",
...
class TestValidateStoryPromotionAlias:
    """validate_story_promotion() is a backward-compatible alias (R-P1-06)."""

    def test_alias_returns_true_empty_reason_on_pass(self, tmp_path: Path) -> None:
        """Alias returns (True, '') matching evaluate_promotion outcome."""
        from _qa_lib import validate_story_promotion
        ...
        ok, reason = validate_story_promotion(receipt_path, "PASS")
        decision = evaluate_promotion(PromotionInputs(receipt_path, "PASS"))
        assert ok == decision.allowed
        assert reason == decision.reason

    def test_alias_returns_false_on_receipt_not_found(self, tmp_path: Path) -> None:
        """Alias returns (False, reason) matching RECEIPT_NOT_FOUND."""
        from _qa_lib import validate_story_promotion
    ...
    def test_alias_returns_false_on_invalid_json(self, tmp_path: Path) -> None:
        """Alias returns (False, reason) matching RECEIPT_PARSE_ERROR."""
        from _qa_lib import validate_story_promotion
    ...
    def test_alias_returns_false_on_receipt_not_passed(self, tmp_path: Path) -> None:
        """Alias returns (False, reason) matching RECEIPT_NOT_PASSED."""
        from _qa_lib import validate_story_promotion
    ...
    def test_alias_returns_false_on_reviewer_rejected(self, tmp_path: Path) -> None:
        """Alias returns (False, reason) matching REVIEWER_REJECTED."""
        from _qa_lib import validate_story_promotion
        ...
        ok, reason = validate_story_promotion(receipt_path, "FAIL")
        decision = evaluate_promotion(PromotionInputs(receipt_path, "FAIL"))
        assert ok == decision.allowed
        assert reason == decision.reason
        assert ok is False

    def test_alias_returns_true_for_warn_reviewer(self, tmp_path: Path) -> None:
        """WARN reviewer is allowed (not FAIL), alias matches evaluate_promotion."""
        from _qa_lib import validate_story_promotion
        ...
        ok, reason = validate_story_promotion(receipt_path, "WARN")
        decision = evaluate_promotion(PromotionInputs(receipt_path, "WARN"))
        assert ok == decision.allowed
        assert reason == decision.reason
        assert ok is True


class TestPromotionEnumValues:
    """Stable string serialization of promotion enums (R-P1-07)."""

    def test_promotion_status_allowed_value(self) -> None:
        """PromotionStatus.ALLOWED.value == 'ALLOWED'."""
        assert PromotionStatus.ALLOWED.value == "ALLOWED"

    def test_promotion_status_blocked_value(self) -> None:
        """PromotionStatus.BLOCKED.value == 'BLOCKED'."""
        assert PromotionStatus.BLOCKED.value == "BLOCKED"

    def test_failure_reason_receipt_not_found_value(self) -> None:
        """PromotionFailureReason.RECEIPT_NOT_FOUND.value == 'RECEIPT_NOT_FOUND'."""
        assert PromotionFailureReason.RECEIPT_NOT_FOUND.value == "RECEIPT_NOT_FOUND"

    def test_failure_reason_receipt_parse_error_value(self) -> None:
        """PromotionFailureReason.RECEIPT_PARSE_ERROR.value == 'RECEIPT_PARSE_ERROR'."""
        assert PromotionFailureReason.RECEIPT_PARSE_ERROR.value == "RECEIPT_PARSE_ERROR"

    def test_failure_reason_receipt_not_passed_value(self) -> None:
        """PromotionFailureReason.RECEIPT_NOT_PASSED.value == 'RECEIPT_NOT_PASSED'."""
        assert PromotionFailureReason.RECEIPT_NOT_PASSED.value == "RECEIPT_NOT_PASSED"

    def test_failure_reason_reviewer_rejected_value(self) -> None:
        """PromotionFailureReason.REVIEWER_REJECTED.value == 'REVIEWER_REJECTED'."""
        assert PromotionFailureReason.REVIEWER_REJECTED.value == "REVIEWER_REJECTED"

    def test_promotion_status_is_str_subclass(self) -> None:
        """PromotionStatus is a str subclass for stable serialization."""
        assert isinstance(PromotionStatus.ALLOWED, str)
        assert isinstance(PromotionStatus.BLOCKED, str)
        assert PromotionStatus.ALLOWED == "ALLOWED"
        assert PromotionStatus.BLOCKED == "BLOCKED"

    def test_failure_reason_is_str_subclass(self) -> None:
        """PromotionFailureReason is a str subclass for stable serialization."""
        assert isinstance(PromotionFailureReason.RECEIPT_NOT_FOUND, str)
        assert isinstance(PromotionFailureReason.REVIEWER_REJECTED, str)
        assert PromotionFailureReason.RECEIPT_NOT_FOUND == "RECEIPT_NOT_FOUND"
        assert PromotionFailureReason.REVIEWER_REJECTED == "REVIEWER_REJECTED"
