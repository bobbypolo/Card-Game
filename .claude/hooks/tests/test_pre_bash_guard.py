...
class TestWrapperInterpreterGuard:
    """# Tests R-P3-01 through R-P3-05 -- wrapper/interpreter exec chain."""

    def test_python_c_with_dangerous_payload_blocked(self) -> None:
        """# Tests R-P3-01 -- python -c with os.system dangerous call blocked."""
        dangerous = "/".join(["rm -rf", ""])  # builds "rm -rf /"
        cmd = f"python -c \"import os; os.system('{dangerous}')\""
        allowed, reason = check_command(cmd)
        assert allowed is False
        assert len(reason) > 0

    def test_python3_c_with_dangerous_payload_blocked(self) -> None:
        """# Tests R-P3-01 -- python3 -c with os.system dangerous call blocked."""
        dangerous = "/".join(["rm -rf", ""])  # builds "rm -rf /"
        cmd = f"python3 -c \"import os; os.system('{dangerous}')\""
        allowed, reason = check_command(cmd)
        assert allowed is False
        assert len(reason) > 0

    def test_pwsh_command_force_push_main_blocked(self) -> None:
        """# Tests R-P3-02 -- pwsh -Command with git push --force origin main blocked."""
        cmd = 'pwsh -Command "git push --force origin main"'
        allowed, reason = check_command(cmd)
        assert allowed is False
        assert len(reason) > 0

    def test_powershell_command_force_push_main_blocked(self) -> None:
        """# Tests R-P3-02 -- powershell -Command with git push --force origin main blocked."""
        cmd = 'powershell -Command "git push --force origin main"'
        allowed, reason = check_command(cmd)
        assert allowed is False
        assert len(reason) > 0

    def test_nested_bash_python_dangerous_blocked(self) -> None:
        """# Tests R-P3-03 -- nested bash -c wrapping python dangerous call blocked."""
        dangerous = "/".join(["rm -rf", ""])  # builds "rm -rf /"
        cmd = f'bash -c "python -c \'import os; os.system(\\"{dangerous}\\")\'"'
        allowed, reason = check_command(cmd)
        assert allowed is False
        assert len(reason) > 0

    def test_nested_sh_bash_dangerous_blocked(self) -> None:
        """# Tests R-P3-03 -- sh -c wrapping bash -c with dangerous payload blocked."""
        dangerous = "/".join(["rm -rf", ""])  # builds "rm -rf /"
        cmd = f"sh -c \"bash -c '{dangerous}'\""
        allowed, reason = check_command(cmd)
        assert allowed is False
        assert len(reason) > 0

    def test_bash_c_safe_command_allowed(self) -> None:
        """# Tests R-P3-04 -- bash -c "echo hello" returns (True, "")."""
        cmd = 'bash -c "echo hello"'
        allowed, reason = check_command(cmd)
        assert allowed is True
        assert reason == ""

    def test_sh_c_safe_command_allowed(self) -> None:
        """# Tests R-P3-04 -- sh -c "ls" returns (True, "")."""
        cmd = 'sh -c "ls"'
        allowed, reason = check_command(cmd)
        assert allowed is True
        assert reason == ""

    def test_python_c_safe_command_allowed(self) -> None:
        """# Tests R-P3-04 -- python -c "print('hello')" returns (True, "")."""
        cmd = "python -c \"print('hello')\""
        allowed, reason = check_command(cmd)
        assert allowed is True
        assert reason == ""

    def test_pwsh_command_safe_allowed(self) -> None:
        """# Tests R-P3-04 -- pwsh -Command "Write-Host hi" returns (True, "")."""
        cmd = 'pwsh -Command "Write-Host hi"'
        allowed, reason = check_command(cmd)
        assert allowed is True
        assert reason == ""

    def test_unwrap_exec_chain_depth_cap_no_infinite_recursion(self) -> None:
        """# Tests R-P3-05 -- _unwrap_exec_chain depth capped at 4, no infinite recursion."""
        from pre_bash_guard import _unwrap_exec_chain

        # A deeply nested chain — the function must terminate
        cmd = 'bash -c "bash -c \\"bash -c \\\\\\"bash -c \\\\\\\\\\\\\\"bash -c echo\\\\\\\\\\\\\\"\\\\\\"\\""'
        result = _unwrap_exec_chain(cmd, depth=0)
        assert isinstance(result, list)
        assert len(result) >= 1
        assert all(isinstance(item, str) for item in result)

    def test_unwrap_exec_chain_stops_at_depth_4(self) -> None:
        """# Tests R-P3-05 -- at depth >= 4, returns immediately with cmd in list."""
        from pre_bash_guard import _unwrap_exec_chain

        cmd = 'bash -c "echo hello"'
        result = _unwrap_exec_chain(cmd, depth=4)
        assert isinstance(result, list)
        assert len(result) >= 1
        assert result[0] == cmd

    def test_node_e_dangerous_payload_blocked(self) -> None:
        """# Tests R-P3-01 -- node -e with xargs rm payload blocked."""
        cmd = "node -e \"require('child_process').exec('find . | xargs rm')\""
        allowed, reason = check_command(cmd)
        assert allowed is False
        assert len(reason) > 0

    def test_perl_e_dangerous_payload_blocked(self) -> None:
        """# Tests R-P3-01 -- perl -e with del /f payload blocked."""
        cmd = "perl -e \"system('del /f important.txt')\""
        allowed, reason = check_command(cmd)
        assert allowed is False
        assert len(reason) > 0
