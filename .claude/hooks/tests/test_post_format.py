...
def run_hook_subagent(file_path: str, cwd: str | None = None) -> subprocess.CompletedProcess:
    """Run the post_format hook with agent_id in stdin (subagent context)."""
    stdin_data = json.dumps({"agent_id": "agent-xyz", "tool_input": {"file_path": file_path}})
    return subprocess.run(
        [sys.executable, str(HOOK_PATH)],
        input=stdin_data,
        capture_output=True,
...
class TestSubagentSkipWriteMarker:
    """# Tests R-P2-04 -- post_format skips write_marker when is_subagent()."""

    def test_subagent_py_file_does_not_create_marker(self, tmp_path: Path) -> None:
        """# Tests R-P2-04 -- subagent context: .py edit does NOT create needs_verify."""
        py_file = tmp_path / "module.py"
        py_file.write_text("x = 1\n", encoding="utf-8")
        result = run_hook_subagent(str(py_file), cwd=str(tmp_path))
        assert result.returncode == 0
        state_file = tmp_path / ".claude" / ".workflow-state.json"
        if state_file.exists():
            state = json.loads(state_file.read_text(encoding="utf-8"))
            assert state.get("needs_verify") is None, (
                "Subagent should NOT create needs_verify marker"
            )

    def test_subagent_ts_file_does_not_create_marker(self, tmp_path: Path) -> None:
        """# Tests R-P2-04 -- subagent context: .ts edit does NOT create needs_verify."""
        ts_file = tmp_path / "module.ts"
        ts_file.write_text("const x = 1;\n", encoding="utf-8")
        result = run_hook_subagent(str(ts_file), cwd=str(tmp_path))
        assert result.returncode == 0
        state_file = tmp_path / ".claude" / ".workflow-state.json"
        if state_file.exists():
            state = json.loads(state_file.read_text(encoding="utf-8"))
            assert state.get("needs_verify") is None, (
                "Subagent should NOT create needs_verify marker for .ts"
            )

    def test_main_agent_py_file_creates_marker(self, tmp_path: Path) -> None:
        """# Tests R-P2-04 -- main agent context (no agent_id): .py edit DOES create marker."""
        py_file = tmp_path / "module.py"
        py_file.write_text("x = 1\n", encoding="utf-8")
        result = run_hook(str(py_file), cwd=str(tmp_path))
        assert result.returncode == 0
        ...
        assert state_file.exists()
        state = json.loads(state_file.read_text(encoding="utf-8"))
        assert state.get("needs_verify") is not None, (
            "Main agent SHOULD create needs_verify marker"
        )

    def test_subagent_check_uses_is_subagent_not_worktree_path(self, tmp_path: Path) -> None:
        """# Tests R-P2-04 -- subagent detection uses agent_id key, not worktree path string."""
        # File not in a worktree path but with agent_id in stdin = subagent
        py_file = tmp_path / "regular_path" / "module.py"
        ...
        if state_file.exists():
            state = json.loads(state_file.read_text(encoding="utf-8"))
            assert state.get("needs_verify") is None, (
                "is_subagent() check should suppress marker even for non-worktree paths"
            )
