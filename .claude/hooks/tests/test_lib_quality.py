...
class TestWorkflowJsonSchema:
    ...
    def test_qa_runner_section(self) -> None:
        ...
        qa = data["qa_runner"]
        keys = set(qa.keys())
        assert keys >= {"production_scan", "custom_steps"}
    ...
    def test_audit_modes_section(self) -> None:
        data = json.loads(WORKFLOW_JSON_PATH.read_text(encoding="utf-8"))
        am = data["audit_modes"]
        assert "quick" in am
        assert "delivery" in am
        assert "full" in am
    ...
    def test_external_scanners_section(self) -> None:
        ...
        assert "pip-audit" in es
        assert es["bandit"]["enabled"] is True
        assert es["semgrep"]["enabled"] is False
        assert es["pip-audit"]["enabled"] is True
...
class TestAuditLogRotation:
    ...
    def test_audit_log_silent_on_os_error(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            # If no exception was raised, verify the log was written
            log_content = log_path.read_text(encoding="utf-8")
            assert "safe-call" in log_content, "audit_log did not write expected entry"
        finally:
            if "CLAUDE_PROJECT_ROOT" in os.environ:
                del os.environ["CLAUDE_PROJECT_ROOT"]
            if "_lib" in sys.modules:
...
class TestLangViolationPatterns:
    """-- LANG_VIOLATION_PATTERNS TypeScript entries"""

    def test_r_p4_07_lang_violation_patterns_has_typescript_key(self) -> None:
        """-- LANG_VIOLATION_PATTERNS has 'typescript' key."""
        from _lib import LANG_VIOLATION_PATTERNS

        assert "typescript" in LANG_VIOLATION_PATTERNS

    def test_r_p4_07_typescript_has_console_log_pattern(self) -> None:
        """-- typescript contains console-log violation ID."""
        from _lib import LANG_VIOLATION_PATTERNS

        ts_patterns = LANG_VIOLATION_PATTERNS["typescript"]
        ids = {vid for _, vid, _, _ in ts_patterns}
        assert "console-log" in ids

    def test_r_p4_07_typescript_has_ts_any_pattern(self) -> None:
        """-- typescript contains ts-any violation ID."""
        from _lib import LANG_VIOLATION_PATTERNS
    ...
    def test_r_p4_07_typescript_has_hardcoded_secret_ts_pattern(self) -> None:
        """-- typescript contains hardcoded-secret-ts violation ID."""
        from _lib import LANG_VIOLATION_PATTERNS

        ts_patterns = LANG_VIOLATION_PATTERNS["typescript"]
        ids = {vid for _, vid, _, _ in ts_patterns}
        assert "hardcoded-secret-ts" in ids

    def test_r_p4_07_typescript_severities_correct(self) -> None:
        """-- typescript violation severity levels are correct."""
        from _lib import LANG_VIOLATION_PATTERNS

        ts_patterns = LANG_VIOLATION_PATTERNS["typescript"]
        sev_map = {vid: sev for _, vid, _, sev in ts_patterns}
        assert sev_map["console-log"] == "warn"
        assert sev_map["ts-any"] == "warn"
        assert sev_map["hardcoded-secret-ts"] == "block"

    def test_r_p4_07_lang_violation_patterns_has_known_language_keys(self) -> None:
        """-- LANG_VIOLATION_PATTERNS contains known language keys."""
        from _lib import LANG_VIOLATION_PATTERNS

        keys = set(LANG_VIOLATION_PATTERNS.keys())
        assert "typescript" in keys, (
            "typescript key missing from LANG_VIOLATION_PATTERNS"
        )
        assert "javascript" in keys, (
            "javascript key missing from LANG_VIOLATION_PATTERNS"
        )

    def test_r_p4_07_typescript_patterns_are_4tuples(self) -> None:
        """-- each typescript entry is a 4-tuple."""
        from _lib import LANG_VIOLATION_PATTERNS

        for entry in LANG_VIOLATION_PATTERNS["typescript"]:
            assert len(entry) == 4
            regex, vid, msg, sev = entry
            assert isinstance(regex, str)
            assert isinstance(vid, str)
    ...
    def test_r_p4_07_console_log_pattern_matches_ts(self) -> None:
        """-- console-log pattern matches console.log call."""
        import re

        from _lib import LANG_VIOLATION_PATTERNS

        ts_patterns = {
            vid: regex for regex, vid, _, _ in LANG_VIOLATION_PATTERNS["typescript"]
        }
        assert re.search(ts_patterns["console-log"], "console.log(x)")
        assert re.search(ts_patterns["console-log"], "  console.log('debug')")

    def test_r_p4_07_ts_any_pattern_matches_explicit_any(self) -> None:
        """-- ts-any pattern matches ': any' type annotation."""
        import re
        ...
        assert re.search(ts_patterns["ts-any"], "function foo(x: any): void")
        assert re.search(ts_patterns["ts-any"], "const data: any = {};")


# ── New tests for STORY-002: write_atomic and is_subagent ─────────────────


class TestWriteAtomic:
    """# Tests R-P2-01"""

    def test_write_atomic_returns_true_on_success(self, tmp_path: Path) -> None:
        """# Tests R-P2-01 -- write_atomic returns True on successful write."""
        from _lib import write_atomic

        target = tmp_path / "output.txt"
        result = write_atomic(target, "hello world")
        assert result is True

    def test_write_atomic_creates_file_with_correct_content(
        self, tmp_path: Path
    ) -> None:
        """# Tests R-P2-01 -- write_atomic writes expected content to target path."""
        from _lib import write_atomic

        target = tmp_path / "output.txt"
        write_atomic(target, "test content")
        assert target.read_text(encoding="utf-8") == "test content"

    def test_write_atomic_writes_bytes_content(self, tmp_path: Path) -> None:
        """# Tests R-P2-01 -- write_atomic supports bytes content."""
        from _lib import write_atomic

        target = tmp_path / "output.bin"
        result = write_atomic(target, b"binary data")
        assert result is True
        assert target.read_bytes() == b"binary data"

    def test_write_atomic_no_temp_file_left_on_success(self, tmp_path: Path) -> None:
        """# Tests R-P2-01 -- no temp file remains after successful write."""
        from _lib import write_atomic

        target = tmp_path / "output.txt"
        write_atomic(target, "content")
        # Temp files use .tmp suffix pattern — none should remain
        tmp_files = list(tmp_path.glob("*.tmp"))
        assert tmp_files == [], f"Temp files remain: {tmp_files}"

    def test_write_atomic_overwrites_existing_file(self, tmp_path: Path) -> None:
        """# Tests R-P2-01 -- write_atomic overwrites an existing file atomically."""
        from _lib import write_atomic
    ...
    def test_write_atomic_returns_false_on_nonexistent_directory(self) -> None:
        """# Tests R-P2-01 -- write_atomic returns False when parent dir does not exist."""
        from _lib import write_atomic

        target = Path("/nonexistent/deep/path/output.txt")
        result = write_atomic(target, "content")
        assert result is False

    def test_write_atomic_does_not_raise_on_failure(self) -> None:
        """# Tests R-P2-01 -- write_atomic catches exceptions and returns False (never raises)."""
        from _lib import write_atomic
        ...
        try:
            ...
            raise AssertionError(f"write_atomic raised {type(exc).__name__}: {exc}")
        assert result is False

    def test_write_atomic_writes_empty_string(self, tmp_path: Path) -> None:
        """# Tests R-P2-01 -- write_atomic handles empty string content."""
        from _lib import write_atomic

        target = tmp_path / "empty.txt"
        result = write_atomic(target, "")
        assert result is True
        assert target.read_text(encoding="utf-8") == ""

    def test_write_atomic_str_and_bytes_types_both_succeed(
        self, tmp_path: Path
    ) -> None:
        """# Tests R-P2-01 -- both str and bytes content types succeed."""
        from _lib import write_atomic

        str_target = tmp_path / "str_out.txt"
        bytes_target = tmp_path / "bytes_out.bin"
        assert write_atomic(str_target, "string content") is True
        assert write_atomic(bytes_target, b"bytes content") is True


class TestIsSubagent:
    """# Tests R-P2-02"""

    def test_is_subagent_returns_true_when_agent_id_present(self) -> None:
        """# Tests R-P2-02 -- is_subagent returns True when agent_id key is in data."""
        from _lib import is_subagent

        data = {"agent_id": "agent-abc123", "tool_input": {}}
        assert is_subagent(data) is True

    def test_is_subagent_returns_false_when_agent_id_absent(self) -> None:
        """# Tests R-P2-02 -- is_subagent returns False when agent_id key is absent."""
        from _lib import is_subagent

        data = {"tool_input": {"command": "echo hello"}, "tool_response": {}}
        assert is_subagent(data) is False

    def test_is_subagent_returns_false_for_empty_dict(self) -> None:
        """# Tests R-P2-02 -- is_subagent returns False for empty dict."""
        from _lib import is_subagent

        assert is_subagent({}) is False

    def test_is_subagent_returns_true_when_agent_id_is_none_value(self) -> None:
        """# Tests R-P2-02 -- is_subagent returns True even when agent_id value is None (key presence matters)."""
        from _lib import is_subagent

        data = {"agent_id": None}
        assert is_subagent(data) is True

    def test_is_subagent_returns_true_when_agent_id_is_empty_string(self) -> None:
        """# Tests R-P2-02 -- is_subagent returns True when agent_id is empty string (key presence matters)."""
        from _lib import is_subagent

        data = {"agent_id": ""}
        assert is_subagent(data) is True

    def test_is_subagent_returns_false_when_only_other_keys(self) -> None:
        """# Tests R-P2-02 -- is_subagent returns False when unrelated keys present."""
        from _lib import is_subagent

        data = {"session_id": "sess-123", "tool_name": "Bash"}
        assert is_subagent(data) is False

    def test_is_subagent_returns_true_with_multiple_keys(self) -> None:
        """# Tests R-P2-02 -- is_subagent returns True when agent_id present among many keys."""
        from _lib import is_subagent
...
# ---------------------------------------------------------------------------
# AuditMode enum tests (R-P2-04, R-P2-05, R-P2-07) — STORY-002 Phase 2
# ---------------------------------------------------------------------------


class TestAuditModeEnum:
    """# Tests R-P2-04, R-P2-05, R-P2-07"""

    def test_audit_mode_is_str_enum(self) -> None:
        """# Tests R-P2-04 -- AuditMode is a str subclass and Enum."""
        from enum import Enum

        from _lib import AuditMode

        assert issubclass(AuditMode, str)
        assert issubclass(AuditMode, Enum)

    def test_audit_mode_quick_value(self) -> None:
        """# Tests R-P2-04 -- AuditMode.QUICK.value == 'quick'."""
        from _lib import AuditMode

        assert AuditMode.QUICK.value == "quick"

    def test_audit_mode_delivery_value(self) -> None:
        """# Tests R-P2-04 -- AuditMode.DELIVERY.value == 'delivery'."""
        from _lib import AuditMode

        assert AuditMode.DELIVERY.value == "delivery"

    def test_audit_mode_full_value(self) -> None:
        """# Tests R-P2-04 -- AuditMode.FULL.value == 'full'."""
        from _lib import AuditMode

        assert AuditMode.FULL.value == "full"

    def test_audit_mode_full_value_evaluates_true(self) -> None:
        """# Tests R-P2-04 -- AuditMode.FULL.value == 'full' evaluates True."""
        from _lib import AuditMode

        result = AuditMode.FULL.value == "full"
        assert result is True

    def test_audit_mode_resolve_quick(self) -> None:
        """# Tests R-P2-05 -- AuditMode.resolve('quick') returns QUICK."""
        from _lib import AuditMode

        assert AuditMode.resolve("quick") == AuditMode.QUICK

    def test_audit_mode_resolve_delivery(self) -> None:
        """# Tests R-P2-05 -- AuditMode.resolve('delivery') returns DELIVERY."""
        from _lib import AuditMode

        assert AuditMode.resolve("delivery") == AuditMode.DELIVERY

    def test_audit_mode_resolve_full(self) -> None:
        """# Tests R-P2-05 -- AuditMode.resolve('full') returns FULL."""
        from _lib import AuditMode

        assert AuditMode.resolve("full") == AuditMode.FULL

    def test_audit_mode_resolve_none_returns_full(self) -> None:
        """# Tests R-P2-05 -- AuditMode.resolve(None) returns FULL."""
        from _lib import AuditMode

        assert AuditMode.resolve(None) == AuditMode.FULL

    def test_audit_mode_resolve_unknown_returns_full(self) -> None:
        """# Tests R-P2-05 -- AuditMode.resolve('unknown') returns FULL."""
        from _lib import AuditMode

        assert AuditMode.resolve("unknown") == AuditMode.FULL

    def test_audit_mode_resolve_empty_string_returns_full(self) -> None:
        """# Tests R-P2-05 -- AuditMode.resolve('') returns FULL."""
        from _lib import AuditMode

        assert AuditMode.resolve("") == AuditMode.FULL

    def test_audit_mode_members_equal_strings(self) -> None:
        """# Tests R-P2-04 -- AuditMode members compare equal to their string values."""
        from _lib import AuditMode

        assert AuditMode.QUICK == "quick"
        assert AuditMode.DELIVERY == "delivery"
