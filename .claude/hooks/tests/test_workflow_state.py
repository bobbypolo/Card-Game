"""Tests for workflow state management. # Tests R-P2-01, R-P2-02, R-P2-04"""
...
class TestWorkflowStateSchema:
    ...
    def test_default_state_has_required_keys(self, tmp_path: Path) -> None:
        """ R-P2-01 -- required keys present, no prod_violations."""
...
class TestEdgeCases:
    ...
    def test_read_partial_state_fills_defaults(self, tmp_path: Path) -> None:
        """ -- partial state filled with defaults."""
        _setup_project(tmp_path)
        state_file = _state_path(tmp_path)
    ...
    def test_read_missing_ralph_section_fills_default(self, tmp_path: Path) -> None:
        """ -- missing ralph section gets defaults."""
        _setup_project(tmp_path)
        state_file = _state_path(tmp_path)
    ...
    def test_update_with_no_kwargs_is_noop(self, tmp_path: Path) -> None:
        """ -- update with no kwargs preserves state."""
        _setup_project(tmp_path)
        lib = _get_lib(tmp_path)
    ...
    def test_write_marker_then_read_marker_roundtrip(self, tmp_path: Path) -> None:
        """ -- write then read marker roundtrip."""
        _setup_project(tmp_path)
        lib = _get_lib(tmp_path)
    ...
    def test_state_file_permissions_are_readable(self, tmp_path: Path) -> None:
        """ -- state file readable after write."""
        _setup_project(tmp_path)
        lib = _get_lib(tmp_path)
        lib.write_workflow_state(lib.read_workflow_state())
...
class TestCheckpointHash:
    """# Tests R-P1-01 R-P1-02 R-P1-03 R-P1-04 -- checkpoint_hash field in ralph state."""

    def test_checkpoint_hash_in_default_state(self, tmp_path: Path) -> None:
        """# Tests R-P1-01 -- DEFAULT_WORKFLOW_STATE['ralph'] contains checkpoint_hash with default ''."""
        _setup_project(tmp_path)
        lib = _get_lib(tmp_path)
        ralph_defaults = lib.DEFAULT_WORKFLOW_STATE["ralph"]
        assert "checkpoint_hash" in ralph_defaults
        assert ralph_defaults["checkpoint_hash"] == ""

    def test_active_worktree_uuid_not_in_default_state(self, tmp_path: Path) -> None:
        """# Tests R-P1-02 -- DEFAULT_WORKFLOW_STATE['ralph'] does NOT contain active_worktree_uuid."""
        _setup_project(tmp_path)
        lib = _get_lib(tmp_path)
        ralph_defaults = lib.DEFAULT_WORKFLOW_STATE["ralph"]
        assert "active_worktree_uuid" not in ralph_defaults

    def test_update_checkpoint_hash_stores_value(self, tmp_path: Path) -> None:
        """# Tests R-P1-03 -- update_workflow_state(ralph={'checkpoint_hash': 'abc123'}) stores value without overwriting consecutive_skips."""
        _setup_project(tmp_path)
        lib = _get_lib(tmp_path)
        # First set consecutive_skips to a known value
        lib.update_workflow_state(ralph={"consecutive_skips": 5})
        # Now update checkpoint_hash only
        result = lib.update_workflow_state(ralph={"checkpoint_hash": "abc123"})
        # checkpoint_hash must be stored
        assert result["ralph"]["checkpoint_hash"] == "abc123"
        # consecutive_skips must NOT be overwritten
        assert result["ralph"]["consecutive_skips"] == 5
        # Verify persisted to file
        loaded = lib.read_workflow_state()
        assert loaded["ralph"]["checkpoint_hash"] == "abc123"
        assert loaded["ralph"]["consecutive_skips"] == 5

    def test_update_checkpoint_hash_clears_to_empty_string(
        self, tmp_path: Path
    ) -> None:
        """# Tests R-P1-04 -- update_workflow_state(ralph={'checkpoint_hash': ''}) clears to empty string."""
        _setup_project(tmp_path)
        lib = _get_lib(tmp_path)
        # First set a hash
        lib.update_workflow_state(ralph={"checkpoint_hash": "abc123def456"})
        # Now clear it
        result = lib.update_workflow_state(ralph={"checkpoint_hash": ""})
        assert result["ralph"]["checkpoint_hash"] == ""
        # Verify persisted
        loaded = lib.read_workflow_state()
        assert loaded["ralph"]["checkpoint_hash"] == ""
