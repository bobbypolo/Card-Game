...
class TestTestCommandDetection:
    ...
    def test_pytest_exit_0_clears_marker(self, tmp_path: Path) -> None:
        ...
        stdin = build_stdin(command="pytest tests/ -v", exit_code=0, stdout="3 passed")
        result = run_hook(stdin, cwd=str(tmp_path))
        assert result.returncode == 0
    ...
    def test_python_m_pytest_exit_0_clears_marker(self, tmp_path: Path) -> None:
        ...
        stdin = build_stdin(
            command="python -m pytest -v", exit_code=0, stdout="5 passed"
        )
        result = run_hook(stdin, cwd=str(tmp_path))
        assert result.returncode == 0
        state = json.loads(state_file.read_text(encoding="utf-8"))
...
class TestMarkerClearing:
    ...
    def test_needs_verify_cleared_on_test_pass(self, tmp_path: Path) -> None:
        ...
        stdin = build_stdin(command="pytest -v", exit_code=0, stdout="10 passed")
        run_hook(stdin, cwd=str(tmp_path))
        state = json.loads(state_file.read_text(encoding="utf-8"))
        assert state["needs_verify"] is None
    ...
    def test_stop_block_count_reset_on_test_pass(self, tmp_path: Path) -> None:
        ...
        stdin = build_stdin(command="pytest -v", exit_code=0, stdout="7 passed")
        run_hook(stdin, cwd=str(tmp_path))
        state = json.loads(state_file.read_text(encoding="utf-8"))
    ...
    def test_both_markers_cleared_simultaneously(self, tmp_path: Path) -> None:
        ...
        stdin = build_stdin(command="pytest", exit_code=0, stdout="2 passed")
        run_hook(stdin, cwd=str(tmp_path))
        state = json.loads(state_file.read_text(encoding="utf-8"))
        assert state["needs_verify"] is None
        assert state["stop_block_count"] == 0
...
class TestClassifyTestExecution:
    """# Tests R-P4-01, R-P4-02, R-P4-03 -- _classify_test_execution() return values."""

    def _import_classify(self):
        """Import _classify_test_execution from post_bash_capture module."""
        import importlib.util

        spec = importlib.util.spec_from_file_location(
            "post_bash_capture", str(HOOK_PATH)
        )
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        return mod._classify_test_execution

    def test_classify_with_passed_count_returns_executed_with_tests(self) -> None:
        """# Tests R-P4-01 -- '3 passed, 0 failed' stdout returns EXECUTED_WITH_TESTS."""
        classify = self._import_classify()
        result = classify("3 passed, 0 failed", "")
        assert result == "EXECUTED_WITH_TESTS"

    def test_classify_with_only_passed_returns_executed_with_tests(self) -> None:
        """# Tests R-P4-01 -- '5 passed' stdout returns EXECUTED_WITH_TESTS."""
        classify = self._import_classify()
        result = classify("5 passed", "")
        assert result == "EXECUTED_WITH_TESTS"

    def test_classify_with_failed_count_returns_executed_with_tests(self) -> None:
        """# Tests R-P4-01 -- '0 passed, 2 failed' stdout returns EXECUTED_WITH_TESTS."""
        classify = self._import_classify()
        result = classify("0 passed, 2 failed", "")
        assert result == "EXECUTED_WITH_TESTS"

    def test_classify_with_error_count_returns_executed_with_tests(self) -> None:
        """# Tests R-P4-01 -- '1 error' in stdout returns EXECUTED_WITH_TESTS."""
        classify = self._import_classify()
        result = classify("1 error", "")
        assert result == "EXECUTED_WITH_TESTS"

    def test_classify_collected_zero_items_returns_executed_zero_tests(self) -> None:
        """# Tests R-P4-02 -- 'collected 0 items' stdout returns EXECUTED_ZERO_TESTS."""
        classify = self._import_classify()
        result = classify("collected 0 items", "")
        assert result == "EXECUTED_ZERO_TESTS"

    def test_classify_no_tests_ran_returns_executed_zero_tests(self) -> None:
        """# Tests R-P4-02 -- 'no tests ran' stdout returns EXECUTED_ZERO_TESTS."""
        classify = self._import_classify()
        result = classify("no tests ran", "")
        assert result == "EXECUTED_ZERO_TESTS"

    def test_classify_empty_stdout_and_stderr_returns_no_evidence(self) -> None:
        """# Tests R-P4-03 -- empty stdout and stderr returns NO_EVIDENCE."""
        classify = self._import_classify()
        result = classify("", "")
        assert result == "NO_EVIDENCE"

    def test_classify_whitespace_only_returns_no_evidence(self) -> None:
        """# Tests R-P4-03 -- whitespace-only stdout returns NO_EVIDENCE."""
        classify = self._import_classify()
        result = classify("   \n  ", "")
        assert result == "NO_EVIDENCE"

    def test_classify_unrelated_output_returns_no_evidence(self) -> None:
        """# Tests R-P4-03 -- unrelated stdout (no test patterns) returns NO_EVIDENCE."""
        classify = self._import_classify()
        result = classify("hello world", "")
        assert result == "NO_EVIDENCE"


class TestMarkerClearingRequiresTestEvidence:
    """# Tests R-P4-04 -- marker clearing requires exit_code==0, is_test_command, EXECUTED_WITH_TESTS."""
    ...
    def test_exit_0_test_cmd_with_evidence_clears_marker(self, tmp_path: Path) -> None:
        """# Tests R-P4-04 -- all three conditions met: marker IS cleared."""
        state_file = self._make_state(tmp_path)
        # Provide stdout with actual test counts (EXECUTED_WITH_TESTS)
        stdin = build_stdin(
        ...
        state = json.loads(state_file.read_text(encoding="utf-8"))
        assert state["needs_verify"] is None, (
            "Marker should be cleared when all three conditions met"
        )

    def test_exit_0_test_cmd_zero_tests_does_not_clear_marker(
        self, tmp_path: Path
    ) -> None:
        """# Tests R-P4-04 -- EXECUTED_ZERO_TESTS blocks marker clearing even with exit 0."""
        state_file = self._make_state(tmp_path)
        stdin = build_stdin(
            command="pytest tests/ -v",
            exit_code=0,
        ...
        state = json.loads(state_file.read_text(encoding="utf-8"))
        assert state["needs_verify"] == "dirty", (
            "Marker should NOT be cleared when zero tests executed"
        )

    def test_exit_0_test_cmd_no_evidence_does_not_clear_marker(
        self, tmp_path: Path
    ) -> None:
        """# Tests R-P4-04 -- NO_EVIDENCE blocks marker clearing even with exit 0."""
        state_file = self._make_state(tmp_path)
        stdin = build_stdin(
            command="pytest tests/ -v",
            exit_code=0,
        ...
        state = json.loads(state_file.read_text(encoding="utf-8"))
        assert state["needs_verify"] == "dirty", (
            "Marker should NOT be cleared when no test evidence in output"
        )

    def test_exit_1_with_evidence_does_not_clear_marker(self, tmp_path: Path) -> None:
        """# Tests R-P4-04 -- exit_code != 0 blocks marker clearing even with test evidence."""
        state_file = self._make_state(tmp_path)
        stdin = build_stdin(
            command="pytest tests/ -v",
            exit_code=1,
        ...
        state = json.loads(state_file.read_text(encoding="utf-8"))
        assert state["needs_verify"] == "dirty", (
            "Marker should NOT be cleared when exit code is non-zero"
        )

    def test_non_test_cmd_with_evidence_does_not_clear_marker(
        self, tmp_path: Path
    ) -> None:
        """# Tests R-P4-04 -- non-test command does not clear marker even if stdout has test-like text."""
        state_file = self._make_state(tmp_path)
        stdin = build_stdin(
        ...
        state = json.loads(state_file.read_text(encoding="utf-8"))
        assert state["needs_verify"] == "dirty", (
            "Marker should NOT be cleared for non-test command"
        )


class TestSubagentSkipClearMarker:
    """# Tests R-P2-03 -- post_bash_capture skips clear_marker when is_subagent()."""

    def test_subagent_test_pass_does_not_clear_marker(self, tmp_path: Path) -> None:
        """# Tests R-P2-03 -- subagent context: test pass does NOT clear needs_verify."""
        state_file = tmp_path / ".claude" / ".workflow-state.json"
        state_file.parent.mkdir(parents=True, exist_ok=True)
        state_file.write_text(
            json.dumps(
        ...
        # Include agent_id to simulate subagent context
        stdin = json.dumps(
            {
        ...
        state = json.loads(state_file.read_text(encoding="utf-8"))
        assert state["needs_verify"] == "dirty", (
            "Subagent should NOT clear needs_verify even on test pass"
        )

    def test_main_agent_test_pass_does_clear_marker(self, tmp_path: Path) -> None:
        """# Tests R-P2-03 -- main agent context: test pass DOES clear needs_verify."""
        state_file = tmp_path / ".claude" / ".workflow-state.json"
        state_file.parent.mkdir(parents=True, exist_ok=True)
        state_file.write_text(
            json.dumps(
        ...
        # No agent_id = main agent context
        stdin = json.dumps(
            {
                "tool_input": {"command": "pytest tests/ -v"},
                "tool_response": {"exitCode": 0, "stdout": "4 passed"},
            }
        )
        result = run_hook(stdin, cwd=str(tmp_path))
        assert result.returncode == 0
        state = json.loads(state_file.read_text(encoding="utf-8"))
        assert state["needs_verify"] is None, (
            "Main agent SHOULD clear needs_verify on test pass"
        )

    def test_subagent_failed_command_still_creates_error_file(
        self, tmp_path: Path
    ) -> None:
        """# Tests R-P2-03 -- subagent with failing command still captures error."""
        stdin = json.dumps(
            {
                "agent_id": "agent-xyz",
        ...
        assert last_error.exists(), (
            "Error file should still be created for subagent failures"
        )
